...
function WebGLGeometryJSON(gl) {
    ...
    this.alpha = 1;
    // -----------------------------------------------------------------------------
    this.create = function (jsonFileData, rawImage, Alpha, nightTexture) {
        ...
        if (nightTexture) {
            this.nightTexture = this.gl.createTexture();
            this.gl.bindTexture(gl.TEXTURE_2D, this.nightTexture);
            this.gl.pixelStorei(gl.UNPACK_FLIP_Y_WEBGL, true);
            this.gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_S, gl.REPEAT);
            this.gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_T, gl.REPEAT);
            this.gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MIN_FILTER, gl.LINEAR);
    ...
    this.render = function (camera, projectionMatrix, shaderProgram, shadowTexture) {
        ...
        if (shadowTexture) {
            ...
            gl.uniform2fv(uniforms.shadowTextureSizeUniform, [1, 1]);
            gl.activeTexture(gl.TEXTURE1);
            gl.bindTexture(gl.TEXTURE_2D, shadowTexture);
        }

        if (this.nightTexture) {
            this.gl.uniform1i(uniforms.earthNightTextureUniform, 2);
