 ...
 */
function WebGLGeometryJSON (gl, LocalshaderProgram) {
	this.gl = gl;
	this.LocalshaderProgram = LocalshaderProgram;
	...
	this.getPosition = function() {
		...
		var Position = new Vector4(this.worldMatrix.getElement(0,3),this.worldMatrix.getElement(1,3),this.worldMatrix.getElement(2,3),1);
		return Position;
	}

	// -----------------------------------------------------------------------------
	this.create = function(jsonFileData, rawImage, testImage) {
		gl.useProgram(LocalshaderProgram)
		...
		if (rawImage) {
			...
			this.gl.texParameterf(gl.TEXTURE_2D, gl.TEXTURE_MAG_FILTER, gl.LINEAR);
			...
			this.gl.uniform1i(LocalshaderProgram.uniforms.textureUniform, 0);
		...
		if (testImage) {
			...
			this.gl.bindTexture(gl.TEXTURE_2D, this.mytexture);
			...
			this.gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGB, gl.RGB, gl.UNSIGNED_BYTE, testImage);

			this.gl.uniform1i(LocalshaderProgram.uniforms.mytextureUniform, 1);
	...
	this.render = function(camera, projectionMatrix, shaderProgram) {
		...
		this.mytexture && gl.bindTexture(gl.TEXTURE_2D, null);
		...
		attributes.vertexMyTexcoordsAttribute && gl.disableVertexAttribArray(attributes.vertexMyTexcoordsAttribute);
	}
}
