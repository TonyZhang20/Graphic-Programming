...
function WebGLGeometryJSON (gl) {
	...
	this.create = function(jsonFileData, rawImage) {
		...
		var mytexcoords = jsonFileData.meshes[0].texturecoords[1];
		...
		this.mytexcoordBuffer = this.gl.createBuffer();
		this.gl.bindBuffer(gl.ARRAY_BUFFER, this.mytexcoordBuffer);
		this.gl.bufferData(gl.ARRAY_BUFFER, new Float32Array(mytexcoords), gl.STATIC_DRAW);
	...
	this.render = function(camera, projectionMatrix, shaderProgram) {
		...
		if(attributes.hasOwnProperty('vertexMyTexcoordsAttribute')){
			gl.bindBuffer(gl.ARRAY_BUFFER, this.mytexcoordBuffer);
			gl.vertexAttribPointer(
				attributes.vertexMyTexcoordsAttribute,
				2,
				gl.FLOAT,
				gl.FALSE,
				0,
			...
			gl.enableVertexAttribArray(attributes.vertexMyTexcoordsAttribute)
		}
