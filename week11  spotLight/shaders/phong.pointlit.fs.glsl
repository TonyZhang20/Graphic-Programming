...
void main(void) {
    ...
    vec3 dSpotLight = vec3(0.0,1.0,0.0) - vec3(0.0,0.0,0.0);
    float theta = dot(dLight, normalize(1.0 * dSpotLight));

    vec3 finalColor = vec3(0.0,0.0,0.0);
    vec3 scrollColor = vec3(0.0,0.0,0.0);
    // todo #9
    ...
    if(theta > 0.8)
    {
        ...
        scrollColor = vec3(
            fract(finalColor.x + uTime),
            fract(finalColor.x + uTime),
            fract(finalColor.x + uTime)
        );
    }

    gl_FragColor = vec4(scrollColor , 1.0);
}
